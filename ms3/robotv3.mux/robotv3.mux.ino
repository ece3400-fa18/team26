...
bool wallDetected(){
  int distance = readSensor(front_wall);
  if (distance > 290){
    Serial.println("front wall sensed");
    Serial.println(readSensor(left_wall));
    if (readSensor(left_wall) > 250) {
      move(right);
      //delay(500);
...
int readSensor(const int which) {
  if(which==front_wall){ //front wall sensor in mux y0
    s0 = 0;
    s1 = 0;
    s2 = 0;    
  }
  if(which==left_wall){ //left wall sensor in mux y1
    s0 = 0;
    s1 = 1;
    s2 = 0;  
  }
  if(which==right_wall){ //right wall sensor in mux y2
    s0 = 0;
    s1 = 1;
    s2 = 0; 
  }
  return analogRead(wall_sensor);
}
